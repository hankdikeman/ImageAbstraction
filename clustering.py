...
def rgb_avg_cluster(count, i, j, track_mat, img):
    if(track_mat.getBool(i, j) and count < 50):
        ...
        # average rgb values
        red = sum([t[0][0] * t[2], b[0][0] * b[2], l[0][0] *
                   l[2], r[0][0] * r[2], img[i, j, 0]]) / sum_n
        green = sum([t[0][1] * t[2], b[0][1] * b[2], l[0][1] *
                     l[2], r[0][1] * r[2], img[i, j, 1]]) / sum_n
        blue = sum([t[0][2] * t[2], b[0][2] * b[2], l[0][2] *
                    l[2], r[0][2] * r[2], img[i, j, 2]]) / sum_n
...
def turnCartoon(img, edgemat, imshape):
    ...
    for i in range(imshape[0]):
        print(chr(27) + "[2J")
        print(str(int(i / imshape[0] * 100)) + "% \tcomplete: " + ("-" * int(
            i * 50 / imshape[0])) + ("." * int((imshape[0] - i) * 50 / imshape[0])))
